...
/**
 * It's `Promise.allSettled()`, but instead of returning an array of
 * {@link PromiseSettledResult}, it checks for any rejected promises and throws
 * the first one it finds, unless if {@link aggregate} is `true`, in which case
 * it throws an {@link AggregateError} containing all the errors, with the error
 * message being a concatenation of all the error messages.
 * If all promises are fulfilled, it returns an array of their values.
 */
export async function allFulfilled<
    const TPromises extends readonly Promise<unknown>[]
>(
    promises: TPromises,
    aggregate = false
