...
export class PrismaCrudGenerator<Types extends SchemaTypes> {
  ...
  private readonly enumRefs = new Map<string, EnumRef<Types, unknown>>();

  public constructor(builder: PothosSchemaTypes.SchemaBuilder<Types>) {
    this.builder = builder;
  }

  public findManyArgs<Name extends string & keyof Types['PrismaTypes']>(modelName: Name) {
  ...
  public getWhere<Name extends string & keyof Types['PrismaTypes']>(
    modelName: Name,
    without?: string[]
  ) {
  ...
  public getWhereUnique<Name extends string & keyof Types['PrismaTypes']>(modelName: Name) {
  ...
  public getOrderBy<Name extends string & keyof Types['PrismaTypes']>(modelName: Name) {
  ...
  public getCreateInput<Name extends string & keyof Types['PrismaTypes']>(
    modelName: Name,
    without?: string[]
  ) {
    const withoutName = (without ?? []).map(name => `Without${capitalize(name)}`).join('');
  ...
  public getCreateManyInput<Name extends string & keyof Types['PrismaTypes']>(
    modelName: Name,
    without?: string[]
  ) {
    const withoutName = (without ?? []).map(name => `Without${capitalize(name)}`).join('');
  ...
  public getUpdateInput<Name extends string & keyof Types['PrismaTypes']>(
    modelName: Name,
    without?: string[]
  ) {
    const withoutName = (without ?? []).map(name => `Without${capitalize(name)}`).join('');
