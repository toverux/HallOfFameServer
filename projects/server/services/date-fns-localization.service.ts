...
export class DateFnsLocalizationService {
    ...
    private readonly dfnsLocaleByCode: Map<string, dfns.Locale> =
        this.buildLocalesMap();

    private readonly supportedLocales = Array.from(
        this.dfnsLocaleByCode.keys()
    );

    /**
     * Gets an appropriate date-fns locale for the given request.
     * If the request does not specify a locale, or the locale is not supported,
     * the {@link defaultLocale} is returned.
     */
    public getLocaleForRequest(req: FastifyRequest): dfns.Locale {
        ...
        return (
            this.dfnsLocaleByCode.get(locale.toLowerCase()) ??
            this.defaultLocale
        );
    }

    public applyTimezoneOffsetOnDateForRequest(
        req: FastifyRequest,
        date: Date
    ): Date {
    ...
    private buildLocalesMap(): Map<string, dfns.Locale> {
        const entries = Object.entries(locales)
            ...
            // Remap locales that are not supported by resolve-accept-language
            // because they use three-letter codes, for now in date-fns this
            // is only the case for "ckb", "Central Kurdish".
            .map(({ locale, code }) => ({
                locale,
                code: code == 'ckb' ? 'ku-IQ' : code
            }))
            // Remap "xx" to "xx-xx" codes as resolve-accept-language expects
            // only the latter format, both meaning the same thing.
            .map(({ locale, code }) => ({
                code: code.includes('-') ? code : `${code}-${code}`,
                locale
            }))
            // Exclude script variants, as resolve-accept-language does not
            // support them. “About 99% of all cases can be covered using the
            // language-country format. We could possibly extend script support
            // in the future given a valid use case, but in the meantime, our
            // goal is to keep this library as simple as possible, while
            // providing the best matches.”
            .filter(
                ({ code }) =>
